class BranchSynchronizer {
    ...
    async syncBranches(srcPlatform, targetPlatform, sourceRepo, targetRepo, sourceOwner, targetOwner) {
        ...
        if (platform === 'bitbucket') return await client.repositories.listBranches({ workspace: owner || 'workspace', repo_slug: repoName });
    ...
    async createBranch(platform, repoName, branchData, owner) {
        ...
        if (!client) throw new Error(`Plateforme non prise en charge: ${platform}`);
        if (platform === 'github') return await client.git.createRef({ owner, repo: repoName, ref: `refs/heads/${branchData.name}`, sha: branchData.commit.sha });
        if (platform === 'gitlab') return await client.Branches.create(repoName, branchData.name, { ref: branchData.commit.sha });
        if (platform === 'bitbucket') return await client.repositories.createBranch({ workspace: owner || 'workspace', repo_slug: repoName, name: branchData.name, target: { hash: branchData.commit.sha } });
